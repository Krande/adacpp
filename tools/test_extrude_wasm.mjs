...
if (out.positions.length !== 8 * 3)
    fail(`expected 24 position floats, got ${out.positions.length}`);
if (out.indices.length !== section.triangles.length)
    fail("index count does not match the section");
if (out.t.length !== 8)
    fail(`expected 8 axial parameters, got ${out.t.length}`);
if (out.rangeLabel.length !== 1 || out.rangeLabel[0] !== 7)
    fail("instance range lost its label");
...
if (Math.abs(vol - expect) > expect * 1e-5)
    fail(`volume ${vol}, expected ${expect} (winding or caps wrong)`);

// Nodes lie on the sweep axis here, so each ring reads a single parameter: 0 at the start, 1 at the end.
for (let i = 0; i < 4; i++) {
    if (Math.abs(out.t[i]) > 1e-6)
        fail(`start ring t=${out.t[i]}, expected 0`);
    if (Math.abs(out.t[4 + i] - 1) > 1e-6)
        fail(`end ring t=${out.t[4 + i]}, expected 1`);
}
