...
static void test_rational_quarter_circle() {
    ...
    for (int i = 0; i <= 10; ++i) {
        double t = i / 10.0;
        if (!close(c.point(t).norm(), 1.0, 1e-9))
            on = false;
    }
...
static void test_rational_bspline_surface_is_cylinder() {
    ...
    s.ctrl = {base[0], base[0] + Vec3{0, 0, 5}, base[1], base[1] + Vec3{0, 0, 5}, base[2], base[2] + Vec3{0, 0, 5}};
...
static void test_revolution_makes_cylinder() {
    ...
    for (int i = 0; i <= 8; ++i)
        for (int j = 0; j <= 4; ++j) {
            ...
            double rho = std::sqrt(p.x * p.x + p.y * p.y);
            if (!close(rho, 3.0, 1e-9) || !close(p.z, (double) j, 1e-9))
                on = false;
        }
