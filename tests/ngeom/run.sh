...
# ngeom_meshopt (EXT_meshopt_compression codecs) + the vendored meshoptimizer — needed by the GLB
# writer's inline meshopt option (ngeom_glb.h -> ngeom_meshopt.h).
mkdir -p "$obj/mo"
g++ -std=c++20 -O2 -c -I third_party/meshoptimizer/src third_party/meshoptimizer/src/*.cpp \
...
# ngeom_glb pulls ngeom_meshopt (the inline meshopt option) -> link the meshopt impl
g++ -std=c++20 -O2 -Wall $INC -I third_party/meshoptimizer/src tests/ngeom/test_ngeom_glb.cpp "$obj/mo"/*.o \
    -o "$obj/test_ngeom_glb"
"$obj/test_ngeom_glb"
