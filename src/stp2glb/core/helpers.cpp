...
bool check_if_string_in_vector(const std::vector<std::string> &vec, const std::string &str) {
    return std::any_of(vec.begin(), vec.end(), [&str](const std::string &filter_name) {
        return std::equal(str.begin(), str.end(), filter_name.begin(), filter_name.end(), [](char a, char b) {
...
TimingContext::~TimingContext() {
    ...
    std::cout << label << " took " << std::fixed << std::setprecision(2) << duration << " seconds." << std::endl;
}
