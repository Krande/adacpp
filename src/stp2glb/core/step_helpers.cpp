...
std::string extractProductNameFromSDR(const Handle(StepShape_ShapeDefinitionRepresentation) & sdr) {
    ...
    Handle(StepRepr_ProductDefinitionShape) pds = Handle(StepRepr_ProductDefinitionShape)::DownCast(def.Value());
    if (pds.IsNull()) {
...
std::string getStepProductNameFromGraph(const Handle(Standard_Transient) & entity, Interface_Graph &theGraph) {
    ...
    std::vector<std::string> path = {"StepShape_ManifoldSolidBrep", "StepShape_AdvancedBrepShapeRepresentation",
    ...
    Handle(StepRepr_RepresentationItem) item = Handle(StepRepr_RepresentationItem)::DownCast(entity);
    ...
    if (!shape_def_rep.IsNull()) {
        auto productName =
            extractProductNameFromSDR(Handle(StepShape_ShapeDefinitionRepresentation)::DownCast(shape_def_rep));
...
std::string getStepProductName(const Handle(Standard_Transient) & entity, Interface_Graph &theGraph) {
    ...
    // ----------------------------------------------------------
    {
        ...
        if (!sdr.IsNull()) {
            ...
            if (!def.Value().IsNull()) {
                ...
                Handle(StepBasic_ProductDefinition) pd = Handle(StepBasic_ProductDefinition)::DownCast(def.Value());
                if (!pd.IsNull()) {
    ...
    // ----------------------------------------------------------
    {
        Handle(StepShape_ShapeRepresentation) shapeRepr = Handle(StepShape_ShapeRepresentation)::DownCast(entity);
        if (!shapeRepr.IsNull()) {
    ...
    }
    {
        ...
        Handle(StepRepr_Representation) repr = Handle(StepRepr_Representation)::DownCast(entity);
        if (!repr.IsNull()) {
            if (!repr->Name().IsNull()) {
...
gp_Trsf get_product_transform(TopoDS_Shape &shape, const Handle(StepBasic_Product) & product) {
    ...
    auto extract_transform_from_placement = [](const Handle(StepGeom_Axis2Placement3d) & placement) -> gp_Trsf {
        ...
        if (!placement.IsNull()) {
            ...
            gp_Dir zDir(placement->Axis()->DirectionRatios()->Value(1), placement->Axis()->DirectionRatios()->Value(2),
                        placement->Axis()->DirectionRatios()->Value(3));

            gp_Dir xDir(placement->RefDirection()->DirectionRatios()->Value(1),
    ...
    if (product->NbFrameOfReference() > 0) {
        ...
        for (Standard_Integer i = 1; i <= frameOfReference->Length(); ++i) {
            ...
            if (!productContext.IsNull()) {
                ...
                Handle(StepRepr_Representation) rep = Handle(StepRepr_Representation)::DownCast(productContext);
                if (!rep.IsNull()) {
                    representation = rep;
    ...
    if (!representation.IsNull()) {
        ...
        for (Standard_Integer i = 1; i <= representation->NbItems(); ++i) {
            ...
            if (item->IsKind(STANDARD_TYPE(StepShape_ShapeRepresentation))) {
                Handle(StepShape_ShapeRepresentation) shapeRep = Handle(StepShape_ShapeRepresentation)::DownCast(item);
...
Interface_EntityIterator MyTypedExpansions(const Handle(Standard_Transient) & rootEntity,
                                           const Handle(Standard_Type) & targetType, const Interface_Graph &theGraph) {
...
Interface_EntityIterator Get_Associated_SolidModel_BiDirectional(const Handle(Standard_Transient) & rootEntity,
                ...
                if (!items.IsNull()) {
                    ...
                    for (Standard_Integer i = 1; i <= nbItems; i++) {
                        ...
                        if (repItem->IsKind(STANDARD_TYPE(StepShape_SolidModel))) {
                            Handle(StepShape_SolidModel) solidModel = Handle(StepShape_SolidModel)::DownCast(repItem);
