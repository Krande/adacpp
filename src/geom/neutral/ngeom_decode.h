...
class Reader {
  public:
    ...
    void seek(const uint8_t *to) { p_ = to; }
    void skip(size_t n) {
        need(n);
        p_ += n;
    }
  ...
  private:
    void need(size_t n) {
        if (p_ + n > end_) throw std::runtime_error("NGEOM: truncated buffer");
    }
    const uint8_t *p_;
    const uint8_t *end_;
};
...
inline NgeomDoc decode(const uint8_t *data, size_t n) {
    ...
    Reader r(data, n);
    if (r.str(8) != "ADANGEOM") throw std::runtime_error("NGEOM: bad magic");
    uint32_t version = (uint32_t)r.i32();
    if (version > NGEOM_VERSION) throw std::runtime_error("NGEOM: unsupported version");
    ...
    for (int ri = 0; ri < record_count; ++ri) {
        ...
        switch (t) {
            ...
            case tag::BSPLINE_CURVE: {
                ...
                for (int i = 0; i < nc; ++i) cp[i] = r.vec3();
                int nk = r.i32();
                ...
                slot.curve = std::make_shared<BSplineCurve>(deg, std::move(cp), std::move(kn),
                                                            std::move(mu), std::move(w), closed);
                break;
            }
            ...
            case tag::BSPLINE_SURFACE: {
                ...
                int ncp = s->nu * s->nv;
                s->ctrl.resize(ncp);
                for (int i = 0; i < ncp; ++i) s->ctrl[i] = r.vec3();
                int nuk = r.i32();
            ...
            case tag::ORIENTED_EDGE: {
                ...
                int has_pc = r.i32();
                if (has_pc) r.i32();  // pcurve ref (ignored: tessellator projects 3D->UV)
            ...
            case tag::EDGE_LOOP: {
                ...
                int ne = r.i32();
                lp->edges.reserve(ne);
                for (int i = 0; i < ne; ++i) lp->edges.push_back(*S.at(r.i32()).oedge);
                slot.loop = lp;
                break;
            }
            case tag::POLY_LOOP: {
                ...
                for (int i = 0; i < np; ++i) lp->polygon.push_back(r.vec3());
                slot.loop = lp;
                break;
            }
            case tag::FACE_BOUND: {
            ...
            case tag::FACE_SURFACE: {
                ...
                int nb = r.i32();
                f->bounds.reserve(nb);
                for (int i = 0; i < nb; ++i) f->bounds.push_back(*S.at(r.i32()).bound);
                slot.face = f;
                break;
            }
            case tag::CONNECTED_FACE_SET: {
                ...
                int nf = r.i32();
                c->faces.reserve(nf);
                for (int i = 0; i < nf; ++i) c->faces.push_back(S.at(r.i32()).face);
                slot.cfs = c;
                break;
            }
