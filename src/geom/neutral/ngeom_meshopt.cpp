...
SimplifiedMesh meshopt_simplify_mesh(const std::vector<float> &positions,
    ...
    const size_t new_count = meshopt_simplify(
        dest.data(), indices.data(), index_count, positions.data(), vertex_count,
        sizeof(float) * 3, target, target_error, meshopt_SimplifyLockBorder, &result_error);
    ...
    for (size_t i = 0; i + 2 < new_count; i += 3) {
        ...
        for (uint32_t idx : {a, b, c}) {
            ...
            if (it == remap.end()) {
                ...
                out.positions.insert(out.positions.end(), positions.begin() + idx * 3,
                                     positions.begin() + idx * 3 + 3);
            } else {
                ni = it->second;
            }
