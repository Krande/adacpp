...
struct Surface {
    ...
    virtual double u_step(double defl, double max_angle) const { return std::numeric_limits<double>::infinity(); }
    virtual double v_step(double defl, double max_angle) const { return std::numeric_limits<double>::infinity(); }
    ...
    virtual std::optional<std::pair<double, double>> v_caps() const { return std::nullopt; }
    // Characteristic model-space size, for chord-deviation tolerancing.
    virtual double approx_size() const { return 1.0; }
    ...
    virtual bool is_quadric() const { return false; }
    ...
    double point_residual(const Vec3 &p) const {
        double u, v;
        if (!uv(p, NAN_HINT, NAN_HINT, u, v)) return 0.0;
...
struct CylinderSurface : Surface {
    ...
    double u_step(double defl, double ma) const override { return angle_step(std::abs(r), defl, ma); }
    double approx_size() const override { return std::abs(r) * 2.0; }
    bool is_quadric() const override { return true; }
};
...
struct ConeSurface : Surface {
    ...
    double radius_at(double v) const { return r0 + v * std::tan(semi_angle); }
    Vec3 point(double u, double v) const override {
    ...
    bool uv(const Vec3 &p, double uh, double, double &u, double &v) const override {
        ...
        if (rxy < 1e-9 * std::max(l.norm(), 1.0)) {
            ...
            if (r0 + v * std::tan(semi_angle) < 0.0) u += M_PI;
            u = wrap_2pi(u);
        }
        return true;
    }
    ...
    double u_step(double defl, double ma) const override { return angle_step(std::abs(r0), defl, ma); }
    double approx_size() const override { return std::max(std::abs(r0), 1.0) * 2.0; }
    bool is_quadric() const override { return true; }
    std::optional<std::pair<double, double>> v_caps() const override {
        ...
        if (std::abs(t) < 1e-12) return std::nullopt;
        double apex = -r0 / t;
        if (apex <= 0.0)
...
struct SphereSurface : Surface {
    ...
    double u_step(double defl, double ma) const override { return angle_step(std::abs(r), defl, ma); }
    double v_step(double defl, double ma) const override { return angle_step(std::abs(r), defl, ma); }
    double approx_size() const override { return std::abs(r) * 2.0; }
    bool is_quadric() const override { return true; }
    std::optional<std::pair<double, double>> v_caps() const override {
...
struct TorusSurface : Surface {
    ...
    double u_step(double defl, double ma) const override { return angle_step(std::abs(R) + std::abs(r), defl, ma); }
    double v_step(double defl, double ma) const override { return angle_step(std::abs(r), defl, ma); }
    double approx_size() const override { return std::abs(R + r) * 2.0; }
    bool is_quadric() const override { return true; }
};
